...
TSubclassOf<ULyraEquipmentDefinition> ULyraInventoryItemInstance::GetEquipmentDefinition() const
{
	const ULyraInventoryItemFragment* fragment = FindFragmentByClass(UInventoryFragment_EquippableItem::StaticClass());
	if (const UInventoryFragment_EquippableItem* equippableFragment = Cast<UInventoryFragment_EquippableItem>(fragment))
	{
		return equippableFragment->EquipmentDefinition;
	}

	return nullptr;
}

FText ULyraInventoryItemInstance::GetItemDisplayName() const
{
	return GetDefault<ULyraInventoryItemDefinition>(ItemDef)->DisplayName;
}
