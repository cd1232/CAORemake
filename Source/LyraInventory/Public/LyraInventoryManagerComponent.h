...
// TODO Craig
// Item Definitions are never spawned, they are used as their CDO so we cannot have properties that change on them like durability
// This would probably have to go on an item instance as they are spawned in when added to the inventory via AddItemDefinition then AddEntry inside the FLyraInventoryList
// Item Definitions are the things that have item fragments, but as far as I can tell these item fragments are not spawned in either so we can't add different stuff to them
// We could have an item fragment that sets up the durability on the item instance through the inventory item fragment function OnInstanceCreated
// 
...
class LYRAINVENTORY_API ULyraInventoryManagerComponent : public UActorComponent
{
	GENERATED_BODY()
