#include "DAOTargetingSelectionTask_AOE.h"
...
UDAOTargetingSelectionTask_AOE::UDAOTargetingSelectionTask_AOE(const FObjectInitializer& ObjectInitializer)
...
void UDAOTargetingSelectionTask_AOE::Execute(const FTargetingRequestHandle& TargetingHandle) const
{
...
void UDAOTargetingSelectionTask_AOE::ExecuteImmediateTrace(const FTargetingRequestHandle& TargetingHandle) const
{
#if ENABLE_DRAW_DEBUG
	...
	if (World && TargetingHandle.IsValid())
	{
		...
		if (ShapeType == ETargetingAOEShape::SourceComponent)
		{
			...
			{
				TARGETING_LOG(Warning, TEXT("UDAOTargetingSelectionTask_AOE::Execute - Failed to find a collision component w/ tag [%s] for a SourceComponent ShapeType."), *ComponentTag.ToString());
			}
		}
		else
		{
			const FCollisionShape CollisionShape = GetCollisionShape();
			FCollisionQueryParams OverlapParams(TEXT("UDAOTargetingSelectionTask_AOE"), SCENE_QUERY_STAT_ONLY(UTargetingSelectionTask_AOE_Shape), false);
...
void UDAOTargetingSelectionTask_AOE::DebugDrawBoundingVolume(const FTargetingRequestHandle& TargetingHandle, const FColor& Color, const FOverlapDatum* OverlapDatum) const
{
#if ENABLE_DRAW_DEBUG
	const UWorld* World = GetSourceContextWorld(TargetingHandle);
...
void UDAOTargetingSelectionTask_AOE::SetShapeType(ETargetingAOEShape InShapeType)
{
	ShapeType = InShapeType;
}

void UDAOTargetingSelectionTask_AOE::SetHalfExtent(FVector InHalfExtent)
{
	HalfExtent = InHalfExtent;
}

void UDAOTargetingSelectionTask_AOE::SetRadius(FScalableFloat InRadius)
{
	Radius = InRadius;
}

void UDAOTargetingSelectionTask_AOE::SetHalfHeight(FScalableFloat InHalfHeight)
{
	HalfHeight = InHalfHeight;
}

void UDAOTargetingSelectionTask_AOE::ExecuteAsyncTrace(const FTargetingRequestHandle& TargetingHandle) const
{
	...
	if (World && TargetingHandle.IsValid())
	{
		...
		FOverlapDelegate Delegate = FOverlapDelegate::CreateUObject(this, &UDAOTargetingSelectionTask_AOE::HandleAsyncOverlapComplete, TargetingHandle);
		if (CollisionObjectTypes.Num() > 0)
		{
			FCollisionObjectQueryParams ObjectParams;
			for (auto Iter = CollisionObjectTypes.CreateConstIterator(); Iter; ++Iter)
...
void UDAOTargetingSelectionTask_AOE::HandleAsyncOverlapComplete(const FTraceHandle& InTraceHandle, FOverlapDatum& InOverlapDatum, FTargetingRequestHandle TargetingHandle) const
{
...
void UDAOTargetingSelectionTask_AOE::ProcessOverlapResults(const FTargetingRequestHandle& TargetingHandle, const TArray<FOverlapResult>& Overlaps) const
{
...
FVector UDAOTargetingSelectionTask_AOE::GetSourceLocation_Implementation(const FTargetingRequestHandle& TargetingHandle) const
{
...
FVector UDAOTargetingSelectionTask_AOE::GetSourceOffset_Implementation(const FTargetingRequestHandle& TargetingHandle) const
{
...
FQuat UDAOTargetingSelectionTask_AOE::GetSourceRotation_Implementation(const FTargetingRequestHandle& TargetingHandle) const
{
	if (const FTargetingSourceContext* SourceContext = FTargetingSourceContext::Find(TargetingHandle))
	{
		if (SourceContext->SourceActor)
...
FCollisionShape UDAOTargetingSelectionTask_AOE::GetCollisionShape() const
{
	switch (ShapeType)
	{
	case ETargetingAOEShape::Box:
...
const UPrimitiveComponent* UDAOTargetingSelectionTask_AOE::GetCollisionComponent(const FTargetingRequestHandle& TargetingHandle) const
{
	if (const FTargetingSourceContext* SourceContext = FTargetingSourceContext::Find(TargetingHandle))
	{
		if (SourceContext->SourceActor)
...
void UDAOTargetingSelectionTask_AOE::InitCollisionParams(const FTargetingRequestHandle& TargetingHandle, FCollisionQueryParams& OutParams) const
{
	if (const FTargetingSourceContext* SourceContext = FTargetingSourceContext::Find(TargetingHandle))
	{
...
#if WITH_EDITOR
bool UDAOTargetingSelectionTask_AOE::CanEditChange(const FProperty* InProperty) const
{
	bool bCanEdit = Super::CanEditChange(InProperty);

	if (bCanEdit && InProperty)
	{
		if (InProperty->GetFName() == GET_MEMBER_NAME_CHECKED(UDAOTargetingSelectionTask_AOE, CollisionChannel))
		{
			return (CollisionProfileName.Name == TEXT("NoCollision") && CollisionObjectTypes.Num() <= 0);
		}

		if (InProperty->GetFName() == GET_MEMBER_NAME_CHECKED(UDAOTargetingSelectionTask_AOE, CollisionProfileName))
		{
			return (CollisionObjectTypes.Num() <= 0);
		}

		if (InProperty->GetFName() == GET_MEMBER_NAME_CHECKED(UDAOTargetingSelectionTask_AOE, HalfExtent))
		{
			return (ShapeType == ETargetingAOEShape::Box || ShapeType == ETargetingAOEShape::Cylinder);
		}

		if (InProperty->GetFName() == GET_MEMBER_NAME_CHECKED(UDAOTargetingSelectionTask_AOE, Radius))
		{
			return (ShapeType == ETargetingAOEShape::Sphere || ShapeType == ETargetingAOEShape::Capsule);
		}

		if (InProperty->GetFName() == GET_MEMBER_NAME_CHECKED(UDAOTargetingSelectionTask_AOE, HalfHeight))
		{
			return (ShapeType == ETargetingAOEShape::Capsule);
		}

		if (InProperty->GetFName() == GET_MEMBER_NAME_CHECKED(UDAOTargetingSelectionTask_AOE, ComponentTag))
		{
...
void UDAOTargetingSelectionTask_AOE::DrawDebug(UTargetingSubsystem* TargetingSubsystem, FTargetingDebugInfo& Info, const FTargetingRequestHandle& TargetingHandle, float XOffset, float YOffset, int32 MinTextRowsToAdvance) const
{
...
void UDAOTargetingSelectionTask_AOE::BuildDebugString(const FTargetingRequestHandle& TargetingHandle, const TArray<FTargetingDefaultResultData>& TargetResults) const
{
#if WITH_EDITORONLY_DATA
	if (UTargetingSubsystem::IsTargetingDebugEnabled())
	{
...
void UDAOTargetingSelectionTask_AOE::ResetDebugString(const FTargetingRequestHandle& TargetingHandle) const
{
#if WITH_EDITORONLY_DATA
	FTargetingDebugData& DebugData = FTargetingDebugData::FindOrAdd(TargetingHandle);
	FString& ScratchPadString = DebugData.DebugScratchPadStrings.FindOrAdd(GetNameSafe(this));
