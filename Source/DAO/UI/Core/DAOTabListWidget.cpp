...
bool UDAOTabListWidget::GetPreregisteredTabInfo(const FName TabNameId, FDAOTabDescriptor& OutTabInfo)
{
	const FDAOTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FDAOTabDescriptor& TabInfo) -> bool
	{
...
void UDAOTabListWidget::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FDAOTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UDAOTabListWidget::RegisterDynamicTab(const FDAOTabDescriptor& TabDescriptor)
{
...
bool UDAOTabListWidget::IsFirstTabActive() const
{
...
bool UDAOTabListWidget::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
    {
...
bool UDAOTabListWidget::IsTabVisible(FName TabId)
{
...
int32 UDAOTabListWidget::GetVisibleTabCount()
{
	int32 Result = 0;
...
void UDAOTabListWidget::NativeConstruct()
{
...
void UDAOTabListWidget::NativeDestruct()
{
	for (FDAOTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void UDAOTabListWidget::HandlePreLinkedSwitcherChanged()
{
	for (const FDAOTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void UDAOTabListWidget::HandlePostLinkedSwitcherChanged()
{
...
void UDAOTabListWidget::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FDAOTabDescriptor* TabInfoPtr = nullptr;
	
	FDAOTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(UDAOTabButtonInterface::StaticClass()))
	{
		...
		{
			IDAOTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
void UDAOTabListWidget::SetupTabs()
{
	for (FDAOTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
